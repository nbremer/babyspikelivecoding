...
const timeScale = d3.scaleLinear()
    .domain([0, numData-1])
    // .range([0, pi2]) //full circle
    .range([0.025 * pi2, 0.975 * pi2]) //small gap in circle
...
d3.csv("data/SciAm_minute_per_day_2014.csv").then((babyData) => {
    ...
    circles.selectAll(".circle")
        ...
        .enter().append("circle")
        .attr("class", "circle")
        // .attr("cx", d => birthScale(d.births) * Math.cos(timeScale(d.time)) ) //radius * cos(angle)
        // .attr("cy", d => birthScale(d.births) * Math.sin(timeScale(d.time)) ) //radius * sin(angle)
